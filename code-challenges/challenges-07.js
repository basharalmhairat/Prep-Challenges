...
const objLat = (obj) => {
    let f = obj.firstName.charAt(0).toUpperCase() + obj.firstName.substr(1);
    let a = obj.lastName.charAt(0).toUpperCase() + obj.lastName.substr(1);


    let result = `my name is ${f} ${a} I am ${obj.age} YO, and I love ${obj.hobby}.`
...
const cvFormatter = (arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        ...
        if (arr[i].yearsOfExperience <= 1) { continue; }
         else 
         {
            let name = (arr[i].firstName ? arr[i].firstName : '');

            let last = (arr[i].LastName ? ' ' + arr[i].LastName : '');

            let cv = { fullName: name + last, tech: `${arr[i].tech}` };
            result.push(cv);
        }
    }
    return result;
